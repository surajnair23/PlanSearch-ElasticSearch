...
import com.csye7255.project.controller.TokenController;
...
public class TokenService {
    ...
    public boolean validateToken(String token) throws RuntimeException{
        ...
        if(!validateTokenMatch(token1)) 
        	throw new BadRequest("Token incorrect");
        if(!authorize(token1)) throw new BadRequest("Token is expired");
        return true;
    }
    
    public boolean validateTokenMatch(String token_value)
    {
        try {
        	TokenController tokenNew = new TokenController();
            if (!token_value.equals(tokenNew.finalToken)) {
                return false;
            }
        }
        catch(BadRequest e)
        {
        	throw new BadRequest("Token incorrect from auth");
        }
        return true;
    }
